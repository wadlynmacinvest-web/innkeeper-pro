...
Full hotel management API with SQLite database
"""

from flask import Flask, jsonify, request, send_from_directory, g
import sqlite3, os, json, datetime, uuid, hashlib
...
CREATE TABLE IF NOT EXISTS bookings (
    ...
    pay_status  TEXT DEFAULT 'unpaid',  -- unpaid | partial | paid
    status      TEXT DEFAULT 'pending', -- pending | confirmed | checkedin | completed | cancelled
    source      TEXT DEFAULT 'walkin',  -- walkin | online | phone
    notes       TEXT,
    created_at  TEXT DEFAULT (datetime('now'))
);
...
CREATE TABLE IF NOT EXISTS receipts (
    id          TEXT PRIMARY KEY,
    booking_id  TEXT REFERENCES bookings(id),
    type        TEXT DEFAULT 'full',  -- full | deposit | partial
    amount      REAL,
...
def _seed():
    ...
    for b in bookings:
        ...
        db.execute("UPDATE rooms SET status=? WHERE id=?", (status_map.get(b[16],'vacant'), b[2]))
...
def dashboard():
    ...
    stats = {
        ...
        'arrivals_today':  query("SELECT COUNT(*) c FROM bookings WHERE checkin=? AND status IN ('pending','confirmed')", (today,), one=True)['c'],
        'departures_today':query("SELECT COUNT(*) c FROM bookings WHERE checkout=? AND status='checkedin'", (today,), one=True)['c'],
        'revenue_today':   query("SELECT COALESCE(SUM(amount),0) s FROM payments WHERE DATE(paid_at)=?", (today,), one=True)['s'],
        'revenue_month':   query("SELECT COALESCE(SUM(amount),0) s FROM payments WHERE strftime('%Y-%m',paid_at)=strftime('%Y-%m','now')", one=True)['s'],
    ...
    today_bdays = query("""
        ...
        WHERE strftime('%m-%d',g.dob)=strftime('%m-%d','now')
          AND g.dob != ''
    """)
    today_annivs = query("""
        SELECT g.first_name||' '||g.last_name name, g.id, b.id booking_id,
        ...
        JOIN bookings b ON b.guest_id=g.id AND b.status='checkedin'
        JOIN rooms r ON r.id=b.room_id
        WHERE strftime('%m-%d',g.anniversary)=strftime('%m-%d','now')
          AND g.anniversary != ''
    """)
...
def guests():
    if request.method == 'GET':
        ...
        if search:
            g = query("SELECT * FROM guests WHERE first_name LIKE ? OR last_name LIKE ? OR email LIKE ? OR phone LIKE ? ORDER BY last_name",
                      (f'%{search}%',)*4)
        else:
    ...
    return ok(query("SELECT * FROM guests WHERE id=?", (gid,), one=True), code=201)
...
def guest_detail(gid):
    ...
    if request.method == 'GET':
        bks = query("""SELECT b.*,r.number room_number,r.type room_type
                       ...
                       WHERE b.guest_id=? ORDER BY b.checkin DESC""", (gid,))
        return ok({**g, 'bookings': bks})

    if request.method == 'PUT':
        d = request.json or {}
        execute("""UPDATE guests SET first_name=?,last_name=?,phone=?,email=?,
                   dob=?,anniversary=?,id_number=? WHERE id=?""",
        ...
        return ok(query("SELECT * FROM guests WHERE id=?", (gid,), one=True))

    execute("DELETE FROM guests WHERE id=?", (gid,))
...
def rooms():
    ...
    if rtype:  conds.append("type=?");   args.append(rtype)
...
def room_detail(rid):
    r = query("SELECT * FROM rooms WHERE id=?", (rid,), one=True)
    ...
    if request.method == 'GET':
        current = query("""SELECT b.*,g.first_name||' '||g.last_name guest_name,g.email,g.phone
                           ...
                           WHERE b.room_id=? AND b.status='checkedin' LIMIT 1""", (rid,), one=True)
        return ok({**r, 'current_booking': current})

    d = request.json or {}
    execute("UPDATE rooms SET status=?,notes=? WHERE id=?",
            (d.get('status', r['status']), d.get('notes', r['notes']), rid))
    return ok(query("SELECT * FROM rooms WHERE id=?", (rid,), one=True))
...
def bookings():
    if request.method == 'GET':
        ...
        args = []
        conds = []
        if status: conds.append("b.status=?"); args.append(status)
        if search:
            conds.append("(g.first_name||' '||g.last_name LIKE ? OR g.email LIKE ? OR b.id LIKE ?)")
            args += [f'%{search}%']*3
        if conds: sql += " WHERE " + " AND ".join(conds)
    ...
    room = query("SELECT * FROM rooms WHERE id=?", (d['room_id'],), one=True)
    ...
    if d.get('status') in ('confirmed','checkedin'):
        ...
        execute("UPDATE rooms SET status=? WHERE id=?", (new_status, d['room_id']))

    return ok(query("""SELECT b.*,g.first_name||' '||g.last_name guest_name,
                              r.number room_number,r.type room_type
                       FROM bookings b JOIN guests g ON g.id=b.guest_id
                       JOIN rooms r ON r.id=b.room_id WHERE b.id=?""", (bid,), one=True))
...
def booking_detail(bid):
    ...
    if request.method == 'GET':
        pmts = query("SELECT * FROM payments WHERE booking_id=? ORDER BY paid_at DESC", (bid,))
        recs = query("SELECT * FROM receipts WHERE booking_id=? ORDER BY issued_at DESC", (bid,))
    ...
    execute("""UPDATE bookings SET status=?,pay_status=?,balance=?,notes=?,pay_method=?
               WHERE id=?""",
    ...
    if new_status != old_status:
        ...
        if room_st:
            execute("UPDATE rooms SET status=? WHERE id=?", (room_st, b['room_id']))
...
def do_checkin(bid):
    b = query("SELECT * FROM bookings WHERE id=?", (bid,), one=True)
    if not b: return err('Booking not found', 404)
    if b['status'] not in ('pending','confirmed'): return err(f"Cannot check in a booking with status '{b['status']}'")
    execute("UPDATE bookings SET status='checkedin' WHERE id=?", (bid,))
    execute("UPDATE rooms SET status='occupied' WHERE id=?", (b['room_id'],))
...
def do_checkout(bid):
    ...
    if d.get('payment_amount'):
        ...
        execute("INSERT INTO payments(id,booking_id,amount,method,reference) VALUES(?,?,?,?,?)",
        ...
        execute("UPDATE bookings SET balance=?,pay_status=? WHERE id=?", (new_balance, new_pay_status, bid))

    execute("UPDATE bookings SET status='completed' WHERE id=?", (bid,))
    execute("UPDATE rooms SET status='vacant' WHERE id=?", (b['room_id'],))

    # Auto-generate receipt
    rid = 'REC-' + str(uuid.uuid4())[:8].upper()
    execute("INSERT INTO receipts(id,booking_id,type,amount) VALUES(?,?,?,?)",
...
def payments():
    ...
    b = query("SELECT * FROM bookings WHERE id=?", (d['booking_id'],), one=True)
    if not b: return err('Booking not found', 404)

    pid = 'PAY-' + str(uuid.uuid4())[:8].upper()
    execute("INSERT INTO payments(id,booking_id,amount,method,reference,notes) VALUES(?,?,?,?,?,?)",
    ...
    execute("UPDATE bookings SET balance=?,pay_status=? WHERE id=?",
...
def celebrations():
    ...
    bdays = query("""
        ...
        WHERE g.dob != ''
        ORDER BY strftime('%m-%d',g.dob)
    """)

    annivs = query("""
        SELECT g.id, g.first_name||' '||g.last_name name, g.email, g.phone, g.anniversary,
               strftime('%m-%d',g.anniversary) md,
               b.id booking_id, r.number room_number, b.status booking_status
        FROM guests g
        LEFT JOIN bookings b ON b.guest_id=g.id AND b.status='checkedin'
        LEFT JOIN rooms r ON r.id=b.room_id
        WHERE g.anniversary != ''
        ORDER BY strftime('%m-%d',g.anniversary)
    """)
...
def send_celeb():
    ...
    g = query("SELECT * FROM guests WHERE id=?", (d['guest_id'],), one=True)
    if not g: return err('Guest not found', 404)

    execute("INSERT INTO celeb_log(guest_id,type,channel,status) VALUES(?,?,?,?)",
...
def report_summary():
    ...
    for i in range(5, -1, -1):
        ...
        ym  = dt.strftime('%Y-%m')
        lbl = dt.strftime('%b')
        rev = query("SELECT COALESCE(SUM(amount),0) s FROM payments WHERE strftime('%Y-%m',paid_at)=?", (ym,), one=True)['s']
        bks = query("SELECT COUNT(*) c FROM bookings WHERE strftime('%Y-%m',created_at)=?", (ym,), one=True)['c']
    ...
    for i in range(29, -1, -1):
        ...
        occ = query("""SELECT COUNT(*) c FROM bookings
                       WHERE checkin<=? AND checkout>? AND status NOT IN ('cancelled')""",
                    (dt, dt), one=True)['c']
        total_rooms = query("SELECT COUNT(*) c FROM rooms", one=True)['c']
